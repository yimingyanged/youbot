...
#include "moveit/kdl_kinematics_plugin_upgrade/chainiksolver_pos_nr_jl_mimic.hpp"
#include <ros/console.h>

namespace KDL
{

ChainIkSolverPos_NR_JL_Mimic_upgrade::ChainIkSolverPos_NR_JL_Mimic_upgrade(const Chain& _chain, const JntArray& _q_min, const JntArray& _q_max,
    ChainFkSolverPos& _fksolver, ChainIkSolverVel& _iksolver, unsigned int _maxiter, double _eps, kdl_kinematics_plugin::IKType ik_type_):
        chain(_chain), q_min(_q_min),
        q_max(_q_max), q_temp(chain.getNrOfJoints()), fksolver(_fksolver), iksolver(_iksolver), delta_q(_chain.getNrOfJoints()), maxiter(_maxiter), eps(_eps),
        q_min_mimic(chain.getNrOfJoints()), q_max_mimic(chain.getNrOfJoints()), ik_type(ik_type_)
{
...
bool ChainIkSolverPos_NR_JL_Mimic_upgrade::setMimicJoints(const std::vector<kdl_kinematics_plugin::JointMimic>& _mimic_joints)
{
...
void ChainIkSolverPos_NR_JL_Mimic_upgrade::qToqMimic(const JntArray& q, JntArray& q_result)
{
...
void ChainIkSolverPos_NR_JL_Mimic_upgrade::qMimicToq(const JntArray& q, JntArray& q_result)
{
  for(std::size_t i=0; i < chain.getNrOfJoints(); ++i)
  {
...
int ChainIkSolverPos_NR_JL_Mimic_upgrade::CartToJnt(const JntArray& q_init, const Frame& p_in, JntArray& q_out)
{
  return CartToJntAdvanced(q_init,p_in,q_out,false);
}

int ChainIkSolverPos_NR_JL_Mimic_upgrade::CartToJntAdvanced(const JntArray& q_init, const Frame& p_in, JntArray& q_out, bool lock_redundant_joints)
{
...
ChainIkSolverPos_NR_JL_Mimic_upgrade::~ChainIkSolverPos_NR_JL_Mimic_upgrade()
{
}
}
