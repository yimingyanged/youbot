...
#include <moveit/kdl_kinematics_plugin_upgrade/kdl_kinematics_plugin.h>
 ...
 */
class ChainIkSolverVel_pinv_mimic_upgrade : public ChainIkSolverVel
{
public:
  /**
   ...
   * default: 150
   */
  explicit ChainIkSolverVel_pinv_mimic_upgrade(const Chain& chain, const Eigen::MatrixXd& Winv_, const JntArray& q_comfortable_,
      int num_mimic_joints =0, int num_redundant_joints =0,
      double eps=1e-5, int maxiter=150, int dim_=kdl_kinematics_plugin::DimIKPositionOrientation6D,
      kdl_kinematics_plugin::IKType ik_type_=kdl_kinematics_plugin::IKPositionOrientation6D, double Kp_=0.3,
      double lambda_=1e-3, bool use_null_=false, bool use_weighting_=false);

  ~ChainIkSolverVel_pinv_mimic_upgrade();
...
private:
  ...
  bool use_nullspace;
  JntArray q_comfortable;
  JntArray q_comfortable_reduced;
  JntArray q_comfortable_locked;

  // This is the set of variable used when solving inverse kinematics
  Eigen::VectorXd xd;
  Eigen::MatrixXd pinvJ, Winv, Winv_locked, C;
  double Kp, lambda;
  ...
  kdl_kinematics_plugin::IKType ik_type;
