...
path(i_path),datasetName(i_datasetName),output_path(i_output_path),dfBlobResponseThreshold(i_dfBlobResponseThreshold)
{
    ...
    m_HeadOfCoffset = vector<int>(m_vocab_size);
    m_RemainingV0barEntires = vector<unsigned int>(m_vocab_size);
...
/*    cout << "List L" << endl;
    ...
    cout << "Num scenes is " << m_num_scenes << endl << endl;
    cin.get();
*/
    ...
    {
     //   cout << "Node " << i << endl;
     //   cout << "C list entires: " << m_C[i].size() << endl;
     //   cout << "Non-cooc edge candidate: " << m_vocab_size - m_NodeIDtoLindex[i] -1 << endl;

        if(m_occurences[i] != 0)    //Defend against clustering bug. Some "orphan" nodes occur 0 times. Thus they have 0 mutual info with all other nodes. Adding them to the graph means the Kruskal's alg will not terminate until almost every edge in the (complete) graph has been examined. So exclude them!
        {
            if((m_HeadOfCoffset[i] >= 0) || (m_RemainingV0barEntires[i] < (m_vocab_size - m_Loffset[i])))
            {
                EdgeWithParams e = GetEdgeWithLargestMutualInfo(i);
                edgeheap.push(e);
                //cout << "Added edge " << e << " from node " << i << endl;
            }
            else
            {
    ...
    while(kruskal.notComplete() && !edgeheap.empty())
    {
        ...
        //Process it
        //cout << "Largest EDGE: " << largest_edge << endl;
        kruskal.ConsiderEdge(largest_edge);
        //cin.get();
        //Now, requeue the next largest edge for the relevant node, if any
        if((m_HeadOfCoffset[largest_edge.a] >= 0) || (m_RemainingV0barEntires[largest_edge.a] < (m_vocab_size - m_Loffset[largest_edge.a])))
        {
            edgeheap.push(GetEdgeWithLargestMutualInfo(largest_edge.a));
        }
        //cout << kruskal.MST.size() << " of " << m_vocab_size << " edges." << endl; //"Requeue takes " <<  HPMOOSTime() - dfTstart << endl;
...
void AcceleratedCLCalculator::OutputCLTree_AndCalculateMarginalsAndConditionals(const vector<EdgeWithParams> &MST)
{
    ...
    //becuase, due to the data structures, the two calculations are hard to disentangle.
    ...
    while(adjacency_list[start_node_id].neighbours.size() == 0) {start_node_id++;}    //Needed due to cluserting bug again. The root can be any node, except an orphan.
    ...
    //outFile << "=========== Now the parent tree ========" << endl;
...
EdgeWithParams AcceleratedCLCalculator::GetEdgeWithLargestMutualInfo(const NodeID &n)
{
    ...
    //Assumes that there is some valid edge originating at n.
    ...
    NodeID nV0_ID;
    //NodeWithOccurences Node_from_L;
    //Node_from_L.nID = 225;
    //Node_from_L.nNv = 0;

    
    /*cout << "running node " << n << endl;
    cout << "Contents of Vbar list: " << endl;
    for(unsigned int k=0;k<m_V0bar[n].size();++k)
    {
        cout << m_V0bar[n][k] << "      " << m_occurences[m_V0bar[n][k]] << endl;
    }

    cout << "L after offset list: " << endl;
    ...
    cout << "Basic L: " << endl;
    for(unsigned int i=m_NodeIDtoLindex[n]+1;i<m_vocab_size;++i)
    {
        cout << m_L[i] << endl;
        if(i-m_NodeIDtoLindex[n]-1>5)
            break;
    }
    */
   ...
   // cout << "running node " << n << "...";
   // cout << "Offset is " << m_Loffset[n] << " out of " << m_vocab_size << endl;
   // cout << "Vbar size is " << m_V0bar[n].size() << endl;
    for(unsigned int i=m_Loffset[n];i<m_vocab_size;++i)
    {
      ...
      //  cout << i << endl;
        ...
        if((m_RemainingV0barEntires[n] == 0) || (find_if(m_C[n].begin(),m_C[n].end(),IsNodeInCList(m_L[i].nID)) == m_C[n].end()))
        {
        ...
        else
        {
            ...
            m_RemainingV0barEntires[n] -= 1;
        }    
    }

   // cout << "Head of V list: " << Node_from_L << " with MI " << dfI_V0 <<  endl;

  /*  cout << "checking C...";
    cout << "Contents of C list: " << endl;
    for(unsigned int k=0;k<m_C[n].size();++k)
    {
        cout << m_C[n][k] << endl;
    }
    */
    ...
    MutualInfo HeadOfC_I = -1.0;

    if(m_HeadOfCoffset[n]>=0)
    {
        //cout << "Looking at item " << m_HeadOfCoffset[n] << endl;
        //C is sorted with the largest entry last, so the head of C is C[end]
        HeadOfC = m_C[n][m_HeadOfCoffset[n]];
        HeadOfC_I = GetMutualInfo(m_num_scenes,m_occurences[n],m_occurences[HeadOfC.nID],HeadOfC.nN_uv);
    }
    if(HeadOfC_I > dfI_V0)
    {
        //Edge with head of C
       // cout << "Picking head of C list: " << HeadOfC << endl;
        //cout << "Getting mutual info for Nu=" << m_occurences[n] << " Nv=" << m_L[m_NodeIDtoLindex[m_C[n][0].nID]].nNv << " Ncc=" << m_C[n][0].nN_uv << endl;
        e.b = HeadOfC.nID;
        e.I = HeadOfC_I;
    ...
    {
        //cout << "Picking head of V list." << endl;
...
{
public:
    TranslateSparseCooccurencesIntoVList(vector<vector<NodeWithMutualInfo> > &m_C,vector<unsigned int> &m_occurences,unsigned int N,Cooccurence FirstInVector )
        : C(m_C), occurences(m_occurences), nN(N),  LastCooccurence(FirstInVector), nCC(0), nProcessed(0)
    {}
    ...
    unsigned int operator() (Cooccurence ThisCooccurence)
    {
        ...
        {
            //cout << "Saw " << LastCooccurence << " " << nCC << " times" << endl;
    ...
    {
        //Get mutual info and insert into C list
        NodeWithMutualInfo nm;
        nm.nID = LastCooccurence.nID2;
        nm.nN_uv = nCC;
        C[LastCooccurence.nID1].push_back(nm);

        //cout << "Added: " << nm << " to the C list of " << LastCooccurence.nID1 << endl;
    }
private:
...
void AcceleratedCLCalculator::MakeSparseCooccurenceStructures()
{
    ...
    //After processing the block, all we need to keep in memory is cocontainer
    {
        ...
        //Really need O(1) add.
        //Update 2 -
        //Switched to Meila's heap idea, implemented as an STXXL vector.
    ...
    //cout << "Memory usage should just have dropped a bit" << endl;
    //cin.get();
    ...
    {
       TranslateSparseCooccurencesIntoVList scan_helper = stxxl::for_each(cocontainer.begin(),cocontainer.end(),TranslateSparseCooccurencesIntoVList(m_C,m_occurences,m_num_scenes, cocontainer[0]),EMC_DEFAULT_NUM_BLOCKS_FOR_SCAN);
    ...
    for(unsigned int i=0;i<m_vocab_size;++i)
    {          
        std::sort(m_C[i].begin(),m_C[i].end(),CList_Comparator_LargestLast(m_occurences,m_num_scenes,m_occurences[i]));
    }
    for(unsigned int i=0;i<m_vocab_size;++i)
    {
        m_RemainingV0barEntires[i] = m_C[i].size();
    }
    ...
    //Initialize m_HeadOfCoffset[i] to point to the end of m_C[i]
    for(unsigned int i=0;i<m_vocab_size;++i)
    {
        m_HeadOfCoffset[i] = m_C[i].size()-1;
    }
...
void AcceleratedCLCalculator::CreateForwardIndexFromInverted(vector<vector<NodeWithOccurences> > &ScenesToWordsIndex,const vector<vector<unsigned int> > &WordToScenesIndex) const
{
    ...
    for(unsigned int s=0;s<m_num_scenes;s++)
    {
        ...
        /*
        cout << "Scene: " << endl;
        for(unsigned int i=0;i<ScenesToWordsIndex[s].size();i++)
        {
            cout << ScenesToWordsIndex[s][i] << endl;
        }
        cin.get();
        */
    }
...
This is not good.
We're going to use a PesudoBayes estimator, which has the form:
...
Bishop discusses various formulae for K in great detail.
...
A possibly better solution she calls the p* estimator.
...
void DfsVisitOrder(NodeID node,int node_parent,vnl_vector<int> &parents,vnl_vector<double> &RelevantConditionals,vnl_vector<double> &RelevantNotConditionals,vector<MarkedNode> &adjacency_list)
{
    if(!adjacency_list[node].marked)
    {
        ...
        if(node_parent != -1)
        {
            ...
            //cout << "Neighbours size: " << adjacency_list[node].neighbours.size() << endl;
            //cout << "Node parent: " << node_parent << endl;
            for(;parent_index<adjacency_list[node].neighbours.size();++parent_index)
            {
              //  cout << "Neighbour " << parent_index << " is " << adjacency_list[node].neighbours[parent_index] << endl;
