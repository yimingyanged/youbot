...
//	ik_group_.move();
	...
	geometry_msgs::PoseStamped halfway_goal = setToGraspPose(getGoal());
	std::vector<double> joints;
	if (!getIK(joints, halfway_goal))
	{
	...
	if (!succeeded)
	{
		ROS_INFO("Direct Grasp: Arm planning NOT succeeded");
		return false;
	}
	if (!moveBase(joints,0.5))
	{
		ROS_INFO("Direct Grasp:	Base movement NOT succeeded");
		//return false;
	}

	geometry_msgs::PoseStamped grasp_goal = setToGraspPose(getGoal());
	if (!getIK(joints, grasp_goal))
	{
		ROS_INFO("Direct Grasp: IK not found for grasp");
		return false;
	}
	...
	std::vector<double> pre_grasp_joints;
	grasp_goal.pose.position.x = grasp_goal.pose.position.x - 0.02;
	ros::param::set("weighting_0",0.0);
	ros::param::set("weighting_1",0.0);
	ros::param::set("weighting_2",0.0);
	if (!getIK(pre_grasp_joints, grasp_goal))
	{
	...
	for (int i = 0; i<5; i++)
	{
		arm_joints[i] = pre_grasp_joints[i];
	}
	plan_group_.setJointValueTarget(arm_joints);
	plan_group_.plan(plan_);
	if (!plan_group_.move())
	{
		ROS_INFO("Direct Grasp: Failed at moving to pre_grasp pose");
		return false;
	}
	...
	for (int i = 0; i<5; i++)
	{
		arm_joints[i] = joints[i];
	}
	plan_group_.setJointValueTarget(arm_joints);
	plan_group_.plan(plan_);
	if (!plan_group_.move())
	{
		ROS_INFO("Direct Grasp: Failed at moving to grasp pose");
		return false;
	}
	if (has_gripper_){
...
bool YoubotManipulator::moveBase(const std::vector<double> & joint_values, double dist_offset)
{
	...
	actionlib::SimpleClientGoalState state = base_ac_.sendGoalAndWait(base_goal, ros::Duration(10),ros::Duration(9));
	if (state.SUCCEEDED)
		return true;
	else
		return false;
...
bool YoubotManipulator::goHome()
{
	...
	base_goal.target_pose.pose.orientation.x = 1;
...
geometry_msgs::PoseStamped YoubotManipulator::setToGraspPose(const geometry_msgs::PoseStamped g)
{
	...
	grasp.M.DoRotY(M_PI);
	grasp.p.z(grasp.p.z() - 0.093);
	grasp.p.x(grasp.p.x() - 0.025);
	...
	grasp_pose.pose.position.z = grasp.p.z()+0.05;
