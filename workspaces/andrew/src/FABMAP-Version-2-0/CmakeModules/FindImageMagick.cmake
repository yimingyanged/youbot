...
# arh hacked this around to work with Windows 'Advanced Installations' (ie.
# compiled from source), by adding the ability to specify a '..._ROOT_DIR'
...
FUNCTION(FIND_IMAGEMAGICK_API component header)
  DBG_MSG("FIND_IMAGEMAGICK_API: comp=${component} head=${header} ARGN=${ARGN} ROOT = ${ImageMagick_ROOT_DIR}")
  ...
  FIND_PATH(ImageMagick_${component}_INCLUDE_DIR
    NAMES ${header}
    PATHS
	  # ARH added these for Windows installations from source
	  ${ImageMagick_ROOT_DIR}
    ${ImageMagick_ROOT_DIR}/magick
	  ${ImageMagick_ROOT_DIR}/Magick++/lib
  ...
  MARK_AS_ADVANCED(ImageMagick_${component}_INCLUDE_DIR)
	DBG_MSG("ImageMagick_${component}_INCLUDE_DIR = ${ImageMagick_${component}_INCLUDE_DIR}")

  FIND_LIBRARY(ImageMagick_${component}_LIBRARY
    NAMES ${ARGN}
    PATHS
	  # ARH added these for Windows installations from source
	  ${ImageMagick_ROOT_DIR}/VisualMagick/bin
	  ${ImageMagick_ROOT_DIR}/VisualMagick/lib

	  # Default places
      "[HKEY_LOCAL_MACHINE\\SOFTWARE\\ImageMagick\\Current;BinPath]/lib"
    DOC "Path to the ImageMagick Magick++ library."
    )
	MARK_AS_ADVANCED(ImageMagick_${component}_LIBRARY)
	DBG_MSG("ImageMagick_${component}_LIBRARY = ${ImageMagick_${component}_LIBRARY}")
...
# ARH added this for Windows installations from source
SET(ImageMagick_ROOT_DIR CACHE PATH "Path to root of source installation for ImageMagick (optional hint)")

IF (WIN32)
  # arh, he say:
  # Magick++ seems to have a dependency on MagickCore, because
  # it needs access to magick/magick-config.h
  # Until we come up with a better way of resolving this, we'll
  # force the inclusion of MagickCore along with Magick++
  list(FIND ImageMagick_FIND_COMPONENTS Magick++ Magick++_REQUESTED)

  IF (Magick++_REQUESTED GREATER -1)
    DBG_MSG("Found request for Magick++, so forcing inclusion of MagickCore too.")
    list(APPEND ImageMagick_FIND_COMPONENTS MagickCore)
    list(REMOVE_DUPLICATES ImageMagick_FIND_COMPONENTS)
  ENDIF (Magick++_REQUESTED GREATER -1)
ENDIF (WIN32)

IF (NOT ImageMagick_FIND_COMPONENTS)
  MESSAGE(AUTHOR_WARNING "You really ought to supply a list of required components to the ImageMagick FIND script!  Talk to Alastair for details.")
ENDIF (NOT ImageMagick_FIND_COMPONENTS)
...
FIND_PATH(ImageMagick_EXECUTABLE_DIR
  ...
  # The previous call deliberately did not search the 'default paths'.
  # If it failed, then this call will look in those places as well.
  FIND_PATH(ImageMagick_EXECUTABLE_DIR
    NAMES mogrify${CMAKE_EXECUTABLE_SUFFIX}
  )
...
FOREACH(component ${ImageMagick_FIND_COMPONENTS}
  ...
  IF(component STREQUAL "Magick++")
    FIND_IMAGEMAGICK_API(Magick++ Magick++.h
      Magick++ CORE_RL_Magick++_ CORE_DB_Magick++_
    )

    # We might also need to look for the

  ELSEIF(component STREQUAL "MagickWand")
    FIND_IMAGEMAGICK_API(MagickWand wand/MagickWand.h
      Wand MagickWand CORE_RL_wand_ CORE_DB_wand_
      )
  ELSEIF(component STREQUAL "MagickCore")
    FIND_IMAGEMAGICK_API(MagickCore magick/MagickCore.h
      Magick MagickCore CORE_RL_magick_ CORE_DB_magick_
      )
...
DBG_MSG("ImageMagick_INCLUDE_DIRS: ${ImageMagick_INCLUDE_DIRS}")
DBG_MSG("ImageMagick_LIBRARIES: ${ImageMagick_LIBRARIES}")
...
IF (ImageMagick_FOUND)
	MARK_AS_ADVANCED(FORCE ImageMagick_ROOT_DIR)
ELSE (ImageMagick_FOUND)
	# Couldn't find ImageMagick, so make the ROOT_DIR variable visible
	# to the user for manual setting.
	MARK_AS_ADVANCED(CLEAR ImageMagick_ROOT_DIR)
